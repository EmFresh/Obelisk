...
struct  XinputDevice
{
	...
	void setTriggerDeadZone(float dz) { deadZoneTrigger = dz; };

	float getStickDeadZone() { return deadZoneStick; };

	float getTriggerDeadZone() { return deadZoneTrigger; };

	virtual void update()
	{
		for(auto bitmask : onButtonAction)
			if(isButtonPressed(bitmask.first))
				onButtonAction[bitmask.first] = true;
			else if(isButtonReleased(bitmask.first))
				onButtonAction[bitmask.first] = false;
	...
	void getVibration(float& L, float& R)
	{
		XINPUT_VIBRATION vibration;
		memset(&vibration, 0, sizeof(XINPUT_VIBRATION));
		L = vibration.wLeftMotorSpeed;
		R = vibration.wRightMotorSpeed;
	}

	void getVibrationL(float& L)
	{
		XINPUT_VIBRATION vibration;
		memset(&vibration, 0, sizeof(XINPUT_VIBRATION));
		L = vibration.wLeftMotorSpeed;
	...
	{
		if(bitmask == CONTROLLER_ANY)return info.Gamepad.wButtons; return info.Gamepad.wButtons == bitmask;
	}
	...
	virtual bool isButtonDown(int bitmask) {
		if(isButtonPressed(bitmask) && !onButtonAction[bitmask])
			return true;

		return false;
	}

	virtual bool isButtonUp(int bitmask) {
		if(isButtonReleased(bitmask) && onButtonAction[bitmask])
			return true;
...
protected:
	...
	std::unordered_map<int, bool> onButtonAction;
};
...
struct  XinputGuitar:public XinputDevice
{
	...
	int getFrets() { return info.Gamepad.wButtons; }

	bool isFretPressed(int bitmask) { return isButtonPressed(bitmask); }

	bool isFretReleased(int bitmask) { return isButtonReleased(bitmask); }

	bool isFretDown(int bitmask) { return isButtonDown(bitmask); }

	bool isFretUp(int bitmask) { return isButtonUp(bitmask); }
private:
...
struct  XinputController:public XinputDevice
{
	...
	void update()
	{
		...
		// 1/32767 = ‭3.051850947599719e-5f
		// 1/255   = ‭0.003921568627451

		float
			low = (float)info.Gamepad.sThumbLX * 0.000030517578125f,
			high = (float)info.Gamepad.sThumbLX * 3.051850947599719e-5f;
		sticks[0].
			x = ((abs(info.Gamepad.sThumbLX < 0 ? low : high) > deadZoneStick) ?
			(info.Gamepad.sThumbLX < 0 ? low : high) : 0);
		low = (float)info.Gamepad.sThumbLY * 0.000030517578125f;
		high = (float)info.Gamepad.sThumbLY * 3.051850947599719e-5f;
		sticks[0].
			y = ((abs(info.Gamepad.sThumbLY < 0 ? low : high) > deadZoneStick) ?
			(info.Gamepad.sThumbLY < 0 ? low : high) : 0);


		low = (float)info.Gamepad.sThumbRX * 0.000030517578125f;
		high = (float)info.Gamepad.sThumbRX * 3.051850947599719e-5f;
		sticks[1].
			x = ((abs(info.Gamepad.sThumbRX < 0 ? low : high) > deadZoneStick) ?
			(info.Gamepad.sThumbRX < 0 ? low : high) : 0);
		low = (float)info.Gamepad.sThumbRY * 0.000030517578125f;
		high = (float)info.Gamepad.sThumbRY * 3.051850947599719e-5f;
		sticks[1].
			y = ((abs(info.Gamepad.sThumbRY < 0 ? low : high) > deadZoneStick) ?
			(info.Gamepad.sThumbRY < 0 ? low : high) : 0);


		triggers = 
		{
			(float)info.Gamepad.bLeftTrigger * 0.003921568627451f,
			(float)info.Gamepad.bRightTrigger * 0.003921568627451f
		};
	}
